...
type DependencytrackClient interface {
	...
	GetMetadataForImage(ctx context.Context, image string) (*model.Image, error)
...
func NewResolver(hookdClient HookdClient,
	...
	return &Resolver{
		...
		searcher:              search.New(teamsearch.New(db), k8sClient, redisClient, openSearchClient, kafkaClient, bigQueryDatasetClient),
		log:                   log,
		database:              db,
