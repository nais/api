...
// 	entries := make([]*model.AuditLog, len(dbe))
// 	for i, entry := range dbe {
// 		entries[i] = &model.AuditLog{
// 			ID:               scalar.AuditLogIdent(entry.ID),
// 			Actor:            entry.Actor,
// 			Action:           audittype.AuditAction(entry.Action),
// 			ComponentName:    logger.ComponentName(entry.ComponentName),
// 			CorrelationID:    entry.CorrelationID.String(),
// 			TargetType:       audittype.AuditLogsTargetType(entry.TargetType),
