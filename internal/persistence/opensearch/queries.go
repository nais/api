...
func Delete(ctx context.Context, input DeleteOpenSearchInput) (*DeleteOpenSearchPayload, error) {
	...
	terminationProtection, found, err := unstructured.NestedBool(openSearch.Object, specTerminationProtection...)
	if err != nil {
		return nil, err
	}
	if found && terminationProtection {
		if err := unstructured.SetNestedField(openSearch.Object, false, specTerminationProtection...); err != nil {
			return nil, err
		}

		if _, err = client.Update(ctx, openSearch, metav1.UpdateOptions{}); err != nil {
