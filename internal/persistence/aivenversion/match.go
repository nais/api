...
// StatusVersion is where the Aiven operator records the version a service is actually
// running, as of aiven/aiven-operator#1280. Both kinds read the same path, so it is
// declared once here rather than beside each kind's spec paths.
var StatusVersion = []string{"status", "version"}

// Declared returns every version the upgrade-path map knows about, sorted. Deriving the
// list rather than maintaining a second one beside it is what keeps validation, matching
// and the newest-version lookup from disagreeing. The sort matters: callers scan the
// result in order, so a raw map range would make matching depend on iteration order.
func Declared[T ~string, U any](paths map[T]U) []T {
	versions := make([]T, 0, len(paths))
	for v := range paths {
...
func MatchPin[T ~string](pin string, declared []T, aivenString func(T) (string, error)) (T, error) {
	...
	for _, value := range declared {
		...
		if err == nil && s == pin {
			return qualified(value, declared, aivenString), nil
		}
	}

	if bare := T("V" + pin); slices.Contains(declared, bare) {
		return qualified(bare, declared, aivenString), nil
	}

	return zero, fmt.Errorf("unsupported pinned version: %q", pin)
}

// qualified returns the VX_Y spelling of a bare VX where one is declared for the same
// Aiven version. The two are one version under two names, and the qualified one is what
// callers should see. Anything already carrying a minor is returned untouched.
func qualified[T ~string](value T, declared []T, aivenString func(T) (string, error)) T {
	if v, err := parseDeclared(value); err != nil || v.hasMinor {
		return value
	}

	want, err := aivenString(value)
	if err != nil {
		return value
	}
	...
	for _, other := range declared {
		...
		if v, err := parseDeclared(other); err != nil || !v.hasMinor {
			continue
		}
		if s, err := aivenString(other); err == nil && s == want {
			return other
		}
	}
	return value
