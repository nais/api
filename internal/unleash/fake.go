...
func (f FakeBifrostClient) Post(ctx context.Context, path string, v any) (*http.Response, error) {
	u, err := f.CreateUnleash(ctx, "devteam")
	if err != nil {
		return nil, err
	}
	fmt.Printf("Unleash created: %v", u)
	return test.Response("200 OK", `{"instance": "test"}`), nil
}
...
func (m *FakeBifrostClient) CreateUnleash(ctx context.Context, team slug.Slug) (*model.Unleash, error) {
	client := m.k8sClient
