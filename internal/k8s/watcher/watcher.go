...
func newWatcher[T Object](mgr *Manager, obj T, settings *watcherSettings, log *slog.Logger) *Watcher[T] {
	w := &Watcher[T]{
		datastore: NewDataStore[T](),
		log:       log,
	}
	for cluster, client := range mgr.managers {
		w.watchers = append(w.watchers, newClusterWatcher(client, cluster, w, obj, settings, log.With("cluster", cluster)))
	}
	return w
}
