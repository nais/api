...
const (
	...
	appCPUUsage         = `avg(rate(container_cpu_usage_seconds_total{namespace=%q, container=%q}[5m]))`
	...
	appMemoryUsage      = `avg(last_over_time(container_memory_working_set_bytes{namespace=%q, container=%q}[5m]))`
