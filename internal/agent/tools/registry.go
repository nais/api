...
- When the user explicitly asks for text/numbers only`,
		Parameters: []Parameter{
			{Name: "chart_type", Type: "string", Description: "The type of chart to render. Currently only 'line' is supported.", Required: true},
			{Name: "title", Type: "string", Description: "A human-readable title for the chart, e.g., 'CPU Usage for my-app'", Required: true},
			{Name: "environment", Type: "string", Description: "The environment to query metrics from (e.g., 'dev', 'prod'). Use the environment from the current context if available.", Required: true},
			{Name: "query", Type: "string", Description: "The Prometheus query to execute. Must be a valid PromQL query.", Required: true},
			{Name: "interval", Type: "string", Description: "Time interval for the query. Valid values: '1h' (1 hour), '6h' (6 hours), '1d' (1 day), '7d' (7 days), '30d' (30 days). Defaults to '1h'.", Required: false},
			{Name: "y_format", Type: "string", Description: "Format type for Y-axis values. Valid values: 'number', 'percentage', 'bytes', 'cpu_cores', 'duration'. Helps the client format the values appropriately.", Required: false},
			{Name: "label_template", Type: "string", Description: "Template string for formatting series labels. Use {label_name} syntax, e.g., '{pod}' or '{pod}/{container}'. If not provided, default label formatting is used.", Required: false},
		},
		Handler: r.executeRenderChart,
	})
}
...
func (r *Registry) executeRenderChart(ctx context.Context, args map[string]any) (any, error) {
	input := RenderChartInput{
		...
		YFormat:       getString(args, "y_format"),
		LabelTemplate: getString(args, "label_template"),
	}
	return r.chartTools.RenderChart(ctx, input)
}
