...
func checkForSecrets(selectionSet ast.SelectionSet, schema *ast.Schema) (bool, string) {
	for _, selection := range selectionSet {
		...
		case *ast.FragmentSpread:
			// Check recursively if the fragment accesses secrets
			if sel.Definition != nil {
				// Check if the fragment type condition is forbidden
				if forbiddenTypes[sel.Definition.TypeCondition] {
					return true, fmt.Sprintf("security policy: fragment '%s' is on type '%s' which contains sensitive data that cannot be accessed via this interface", sel.Name, sel.Definition.TypeCondition)
				}

				if found, reason := checkForSecrets(sel.Definition.SelectionSet, schema); found {
					return true, reason
				}
			} else if strings.Contains(strings.ToLower(sel.Name), "secret") {
				// Fallback heuristic if definition is missing
...
func calculateQueryDepth(selectionSet ast.SelectionSet, currentDepth int) int {
	...
	for _, selection := range selectionSet {
		...
		case *ast.FragmentSpread:
			if sel.Definition != nil {
				childDepth = calculateQueryDepth(sel.Definition.SelectionSet, currentDepth)
			} else {
