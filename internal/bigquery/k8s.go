...
func (c *Client) BigQueryDatasets(ctx context.Context, teamSlug slug.Slug) ([]*model.BigQueryDataset, error) {
	bigQueryDatasetListOpsCounter.Add(ctx, 1)
	...
	for env, infs := range c.informers {
		...
		if err != nil {
			bigQueryDatasetListErrorCounter.Add(ctx, 1)
...
func (c *Client) BigQueryDataset(ctx context.Context, env string, slug slug.Slug, name string) (*model.BigQueryDataset, error) {
	bigQueryDatasetOpsCounter.Add(ctx, 1)
	inf, exists := c.informers[env]
	if !exists {
		bigQueryDatasetErrorCounter.Add(ctx, 1)
...
func (c *Client) CostForBiqQueryDataset(ctx context.Context, env string, teamSlug slug.Slug, ownerName string) float64 {
	bigQueryDatasetCostOpsCounter.Add(ctx, 1)
	cost := 0.0
	...
	if sum, err := c.db.CostForInstance(ctx, "BigQuery", from, to, teamSlug, ownerName, env); err != nil {
		bigQueryDatasetCostOpsErrorCounter.Add(ctx, 1)
