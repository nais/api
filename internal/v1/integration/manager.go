...
func newGQLRunner(ctx context.Context, config *Config, db database.Database, topic graphv1.PubsubTopic, k8sRunner *apiRunner.K8s) (spec.Runner, error) {
	...
	watcherMgr, err := watcher.NewManager(k8sRunner.Scheme, "dev-nais", watcher.Config{
		Clusters: clusters(),
	...
	graphMiddleware, err := api.ConfigureV1Graph(ctx, true, watcherMgr, db, nil, nil, config.TenantName, clusters(), fakefeedback.NewClient(log), log)
	if err != nil {
