...
func LogStream(ctx context.Context, filter *WorkloadLogSubscriptionFilter) (<-chan *WorkloadLogLine, error) {
	...
	clientSets := fromContext(ctx).clients
	k8sClientSet, exists := clientSets[filter.Environment]
	if !exists {
	...
	for _, pod := range pods {
		...
		go func() {
			...
			if err != nil {
				log.WithError(err).Errorf("getting logs")
				return
			}
			defer func() {
				if err := logs.Close(); err != nil {
					log.WithError(err).Errorf("closing logs")
				}
			}()
