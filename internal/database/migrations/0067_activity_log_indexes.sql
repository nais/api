-- +goose Up
-- Recreate the materialized view with team_slug cast to the slug domain so
-- that both sides of the activity_log_combined_view UNION ALL have identical
-- types. Without matching types PostgreSQL cannot push WHERE predicates down
-- into the individual scans, causing full sequential scans of all rows even
-- when filtering by a single team.
...
WITH
	k8s_scale_events AS (
		SELECT
			...
			involved_namespace::slug AS team_slug,
	...
	k8s_audit_events AS (
		SELECT
			...
			triggered_at AS created_at,
			data ->> 'user' AS actor,
			'CLUSTER_AUDIT' AS action,
			'CLUSTER_AUDIT' AS resource_type,
			involved_name AS resource_name,
			involved_namespace::slug AS team_slug,
			(
		...
		FROM
			k8s_events
		WHERE
			involved_kind NOT IN ('HorizontalPodAutoscaler')
			AND data ->> 'user' IS NOT NULL
	),
	deployments AS (
		SELECT
