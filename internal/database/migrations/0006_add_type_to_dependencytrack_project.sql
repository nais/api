-- +goose Up
DELETE FROM dependencytrack_projects T1 USING dependencytrack_projects T2
WHERE
	T1.created_at < T2.created_at -- delete the older versions
	AND T1.environment = T2.environment
	AND T1.team_slug = T2.team_slug
	AND T1.app = T2.app
;

CREATE TYPE workload_type AS ENUM('app', 'naisjob')
;

ALTER TABLE dependencytrack_projects
RENAME COLUMN app TO workload
;

ALTER TABLE dependencytrack_projects
ADD COLUMN workload_type workload_type
;

UPDATE dependencytrack_projects
SET
	workload_type = 'app'
;

ALTER TABLE dependencytrack_projects
ALTER COLUMN workload_type
SET NOT NULL
;
